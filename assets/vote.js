...
function updateVoteLink(link) {
    ...
    if (nextVoteTime > Date.now()) {
        ...
        link.querySelector('.vote-timer').classList.remove('d-none');
    } else {
        link.classList.remove('btn-disabled');
        link.querySelector('.vote-timer').innerText = '';
        link.querySelector('.vote-timer').classList.add('d-none');
...
function initVote() {
    document.querySelectorAll('[data-vote-url]').forEach(function (el) {
        ...
        const clickListener = function (ev) {
            ...
            if ((voteTime && voteTime > Date.now()) || el.classList.contains('btn-disabled')) {
                ev.preventDefault();
                return;
            }
